...
local function isHaveSelfGuildQueueDoing(buildServerData,queueType)
	if g_cityBattlePlayerData.getCampId() ~= 0 then
		...
		for k , v in pairs(currentQueueDatas.Queue) do
			assert(v.to_map_id ~= 0, "error : to_map_id == 0 ")
			if buildServerData.id == v.to_map_id then
				if v.player_id ~= m_MyPlayerData.player_id and v.camp_id == g_cityBattlePlayerData.getCampId() and v.type == queueType then
					return v
				end
			end
		end
...
local function isHaveOtherQueueDoing(buildServerData,queueType)
	...
	for k , v in pairs(currentQueueDatas.Queue) do
		assert(v.to_map_id ~= 0, "error : to_map_id == 0 ")
		if buildServerData.id == v.to_map_id then
			if v.player_id ~= m_MyPlayerData.player_id and (v.camp_id == 0 or v.camp_id ~= g_cityBattlePlayerData.getCampId())	and v.type == queueType then
				return v
			end
		end
	end
...
function create_with_buildServerData(buildServerData , tipMenuId)
	...
	m_MyPlayerData = g_cityBattlePlayerData.GetData()
	...
	elseif map_element_origin_id == HelperMD.m_MapOriginType.player_home then--------------------------------------------------------------城堡
		...
		elseif isSelfGuild(buildServerData) then
			--自己联盟的其他玩家城堡
			local queueSD = require "game.mapcitybattle.worldMapLayer_bigMap".getSelfQueueDoing_bigTileIndex_queueType(buildServerData.id, QueueHelperMD.QueueTypes.TYPE_CITYASSIST_ING)
			if queueSD then
				--自己在援助
				menu_Key = 5
			else
	...
	elseif map_element_origin_id == HelperMD.m_MapOriginType.king_castle then--------------------------------------------------------------王城
		if g_cityBattlePlayerData.getCampId() == 0 then
			...
			if g_kingInfo.isKingBattleStarted() then
				...
				--王城非交战中
				if buildServerData.camp_id == 0 then
					--无领主
					menu_Key = 1
				else
					--有领主
	...
	elseif map_element_origin_id == HelperMD.m_MapOriginType.guild_war_gongchengchui then--------------------------------------------------------------攻城锤
		...
		print("g_cityBattleInfoData.IsAttacker():",g_cityBattleInfoData.IsAttacker())
		if g_cityBattleInfoData.IsAttacker() then --攻击方
			
			local configId = 1
			if g_cityBattlePlayerData.getCampId() == 1 then
				configId = 1
			elseif g_cityBattlePlayerData.getCampId() == 2 then
				configId = 4
			elseif g_cityBattlePlayerData.getCampId() == 3 then
				configId = 9
			end
			local gate_1_InitData = g_data.city_battle_map_config[configId]
			...
			print("gateIsBroken,",gate_1_IsBroken)
			if gate_1_IsBroken then
				menu_Key = 4
			else
			
				if g_cityBattlePlayerData.getCampId() == tonumber(buildServerData.area) then --自己阵营的
	...
	elseif map_element_origin_id == HelperMD.m_MapOriginType.guild_war_gate then--------------------------------------------------------------城门
		if g_cityBattleInfoData.IsAttacker() and buildServerData.durability > 0  then --攻击方
			menu_Key = 1
		else --防守方
			menu_Key = 2
		end
	elseif map_element_origin_id == HelperMD.m_MapOriginType.guild_war_chuangnu then--------------------------------------------------------------床弩
		...
		if not g_cityBattleInfoData.IsAttacker() then --攻击方
...
--				local currentBuildIdx = g_cityBattleMapSpBuildData.getLocalSpBuildDataBy_xy(buildServerData.x,buildServerData.y).build_num
...
--				end
	...
	elseif map_element_origin_id == HelperMD.m_MapOriginType.guild_war_yunti then--------------------------------------------------------------云梯
		if g_cityBattleInfoData.IsAttacker() then --攻击方
   		...
   		if buildServerData.resource >= maxValue then
   			...
   			if g_cityBattlePlayerData.getCampId() == tonumber(buildServerData.area) then --自己阵营的云梯
	...
	elseif map_element_origin_id == HelperMD.m_MapOriginType.guild_war_toushiche then--------------------------------------------------------------投石车
		if g_cityBattleInfoData.IsSelfOccupationArea(buildServerData.area) then --投石车属于我方
			if buildServerData.player_id == 0 then
				--无人
				menu_Key = 1
			elseif isMySelf(buildServerData) then
			...
			elseif isSelfGuild(buildServerData) then
				...
				--其他人
				menu_Key = 4
				if not g_cityBattleInfoData.IsDoorMap() then --内城战投石车可争夺
					menu_Key = 5
				end
			end
		else --投石车属于对方 不可占领
	...
	if buildServerData.camp_id ~= 0 then
		--guildData_belong = bigMap.getCurrentAreaDatas().Camp[tostring(buildServerData.camp_id)]
	end
	...
	local function onCellButton(sender, eventType)
		...
		elseif eventType == ccui.TouchEventType.ended then
			if open_animation_completed then
				require("game.mapcitybattle.worldMapLayer_smallMenuClick").onClick(
		...
		if map_element_origin_id == HelperMD.m_MapOriginType.player_home and isMySelf(buildServerData) then
			...
			local function onSaveIndexButton(sender, eventType)
				if eventType == ccui.TouchEventType.ended then
					require("game.mapcitybattle.worldMapLayer_pecialClick").onClick_SaveIndex(cc.p(buildServerData.x,buildServerData.y), g_data.map_element[tonumber(buildServerData.map_element_id)], buildServerData)
					bigMap.closeSmallMenu()
				end
			end
...
function create_with_queueServerData(queueServerData , tipMenuId)
	...
	local guildData_belong = nil
	if queueServerData.camp_id ~= 0 then
		--guildData_belong = bigMap.getCurrentQueueDatas().Camp[tostring(queueServerData.camp_id)]
	end
...
function create_with_bigTileIndex(bigTileIndex , tipMenuId)
	...
	--最终需要的key
	local menu_Key = 1 --g_cityBattlePlayerData.isAllianceManager() and 2 or 1
	...
	do
		...
		local function onSaveIndexButton(sender, eventType)
			if eventType == ccui.TouchEventType.ended then
				require("game.mapcitybattle.worldMapLayer_pecialClick").onClick_SaveIndex(bigTileIndex, nil, nil)
				bigMap.closeSmallMenu()
			end
		end
		bottom:getChildByName("Image_4"):addTouchEventListener(onSaveIndexButton)
...
function setTipMenuID( tipMenuId )
	local widget = require "game.mapcitybattle.worldMapLayer_bigMap".getSmallMenu()
	if widget then
