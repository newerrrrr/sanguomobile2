...
local c_move_city_gem_count = 2000
...
function create(map_element_id, bigTileIndex, callbackOK, callbackCancle, tp)
	...
	local function checkMask_color()
		if mask_single_size then
			...
			for k , v in ipairs(mask_array) do
				...
				if w_p then
					local tileData = require("game.maplayer.worldMapLayer_bigMap").getTileData_WorldPosition(w_p)
					if tileData then
						if tp == m_MenuMode.move_city then
							--迁城时自己的主城忽略判定
							local build_id = tileData:getCustomName()
							if build_id and build_id ~= "" then
								local serverData = require("game.maplayer.worldMapLayer_bigMap").getCurrentAreaDatas().Map[build_id]
								if serverData == nil 
									or serverData.map_element_origin_id ~= HelperMD.m_MapOriginType.player_home 
									or tonumber(playerData.id) ~= tonumber(serverData.player_id) 
	...
	local function update_AutoMove(dt)
		if origin_bigTileIndex then
			local origin_center_Ingex = require("game.maplayer.worldMapLayer_bigMap").getBigTileIndex_CurrentLookAt()
			require("game.maplayer.worldMapLayer_bigMap").offsetAddBigTileIndex_Manual(auto_offset_index)
			local new_center_Ingex = require("game.maplayer.worldMapLayer_bigMap").getBigTileIndex_CurrentLookAt()
	...
	local function checkOutViewPort()
		...
		local viewSize = require("game.maplayer.worldMapLayer_bigMap").getMapScrollViewSize()
		local viewPos = require("game.maplayer.worldMapLayer_bigMap").getWorldPointInMapScrollView(touch_move_Location_Point)
		local isOpen = false
	...
	local function onTouchBegan(touch, event)
		...
		if ret:isVisible() then
			...
			if r == true then
				origin_bigTileIndex = HelperMD.position_2_bigTileIndex(require("game.maplayer.worldMapLayer_bigMap").worldPosition_2_position(touch:getLocation()))
				require("game.maplayer.worldMapLayer_bigMap").setMapScrollViewTouchEnabled(false)
	...
	local function onTouchMoved(touch, event)
		if origin_bigTileIndex then
			...
			local new_bigTileIndex = HelperMD.position_2_bigTileIndex(require("game.maplayer.worldMapLayer_bigMap").worldPosition_2_position(touch_move_Location_Point))
			if new_bigTileIndex.x ~= origin_bigTileIndex.x or new_bigTileIndex.y ~= origin_bigTileIndex.y then
				...
				local offset_position = cc.pSub(HelperMD.bigTileIndex_2_position(new_bigTileIndex), HelperMD.bigTileIndex_2_position(origin_bigTileIndex))
				ret:setPosition(cc.p(ret:getPositionX() + offset_position.x, ret:getPositionY() + offset_position.y))
				origin_bigTileIndex = HelperMD.position_2_bigTileIndex(require("game.maplayer.worldMapLayer_bigMap").worldPosition_2_position(touch_move_Location_Point))
				checkMask_color()
			end
			checkOutViewPort()
		end
	...
	local function onTouchEnded(touch, event)
		...
		require("game.maplayer.worldMapLayer_bigMap").setMapScrollViewTouchEnabled(true)
	end
