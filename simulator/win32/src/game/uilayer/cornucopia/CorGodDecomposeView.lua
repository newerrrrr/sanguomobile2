...
local CorGodDecomposeView = class("CorGodDecomposeView",require("game.uilayer.base.BaseLayer"))

function CorGodDecomposeView:ctor()
  CorGodDecomposeView.super.ctor(self)
  print("CorGodDecomposeView:ctor")

  self.equipArray = SmithyData:instance():getEquipForDecompose()
  self.selectedArray = {}
end 

function CorGodDecomposeView:onEnter()
  print("CorGodDecomposeView:onEnter")
...
function CorGodDecomposeView:onExit() 
  print("CorGodDecomposeView:onExit") 
end 

function CorGodDecomposeView:initBinding(scaleNode)
...
function CorGodDecomposeView:showSelectedEquips(equipArray)
  --reset 
...
function CorGodDecomposeView:showMaterialsList()
  ...
  if dataLen > 0 then 
    ...
    for i=1, itemCount do 
      ...
      for k=1, col do 
        idx = (i-1) * col + k 
        if idx <= dataLen then 
          child = g_gameTools.getImageView(self.gainedMat[idx][1], self.gainedMat[idx][2], self.gainedMat[idx][3], true)
          if child then 
...
function CorGodDecomposeView:clearAndReset()
  self.equipArray = SmithyData:instance():getEquipForDecompose()
...
--手动添加
function CorGodDecomposeView:onManualAdd(sender) 
...
--自动添加
function CorGodDecomposeView:onBatchAdd()
  print("onBatchAdd")
  g_musicManager.playEffect(g_SOUNDS_SURE_PATH)
...
--开始分解
function CorGodDecomposeView:onDecompose()
  print("onDecompose") 
  g_musicManager.playEffect(g_SOUNDS_SURE_PATH)
...
function CorGodDecomposeView:playDecomposeAnim(target1, targetTbl, animEndCallback1, animEndCallback2)
...
function CorGodDecomposeView:getInAni()
  return false 
end

return  CorGodDecomposeView
