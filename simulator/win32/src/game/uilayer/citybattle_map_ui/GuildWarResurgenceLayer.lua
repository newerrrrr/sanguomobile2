	...
	self._baseNode:getChildByName("content_popup"):getChildByName("btn_confirm"):addClickEventListener(function()
		...
		local function onRecv(result, msgData)
	      ...
	      if(result==true)then
		       ...
		       require "game.mapguildwar.changeMapScene".gotoWorld_BigTileIndex(g_cityBattlePlayerData.GetPosition(),function()
...
function GuildWarResurgenceLayer:updateView()
	...
	if g_cityBattlePlayerData.GetData().is_dead ~= 1 then 
		self:removeFromParent()
		return
	end
	...
	local fhTime = g_cityBattlePlayerData.GetData().dead_time + tonumber(g_data.country_basic_setting[51].data)
	local function updateTimeStr()
		...
		local costNum = timeLeft * costPrice / tonumber(g_data.country_basic_setting[51].data)
