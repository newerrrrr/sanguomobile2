...
function ChatLayer:ctor(chatType)
  ...
  --战场聊天和城战聊天共用一个窗口(语音聊天),如果处于战斗中则备份下聊天类型(点击菜单按钮时用到)
  self.isInBattle, self.typeOfBattle = ChatMode.isInBattle() 
  print("isInBattle, typeOfBattle", self.isInBattle, self.typeOfBattle)

  --如果战场/城战聊天未开放, 则默认打开世界聊天
  if not self.isInBattle and self.curType == self.typeOfBattle then 
...
function ChatLayer:initBinding(scaleNode)
  ...
  self.btnCamp = scaleNode:getChildByName("Button_4")    
  self.btnBlackName =  scaleNode:getChildByName("Button_5") 
  ...
  scaleNode:getChildByName("Text_4"):setString(g_tr("chat_camp"))
  ...
  if self.isInBattle and self.typeOfBattle == ChatType.CityBattle then 
    scaleNode:getChildByName("Text_3"):setString(g_tr("chat_city_battle"))
  end 
  ...
  self:regBtnCallback(self.btnCamp, handler(self, self.onChatCamp))
  ...
  --如果未开战则隐藏战场聊天界面
  self.btnBattle:setVisible(self.isInBattle)
  scaleNode:getChildByName("Text_3"):setVisible(self.isInBattle)

  --如果玩家无阵营则隐藏阵营聊天界面
  local isCampVisible = g_PlayerMode.GetData().camp_id and g_PlayerMode.GetData().camp_id > 0 
  self.btnCamp:setVisible(isCampVisible)
  scaleNode:getChildByName("Text_4"):setVisible(isCampVisible)
...
function ChatLayer:highlightMenu(chatType) 
  ...
  self.btnBattle:setHighlighted(chatType == self.typeOfBattle)
  self.btnCamp:setHighlighted(chatType == ChatType.Camp)
  ...
  if self.nodeSend:isVisible() then 
    --文本/语音开关, 默认文本输入
    if chatType == self.typeOfBattle then 
      self.nodeSend:getChildByName("Image_yuyin"):setVisible(true)
      self.nodeSend:getChildByName("Image_jianpan"):setVisible(false)
    else 
      self.nodeSend:getChildByName("Image_yuyin"):setVisible(false)
      self.nodeSend:getChildByName("Image_jianpan"):setVisible(true)      
    end 
...
function ChatLayer:insertChatItem(itemData, index) 
  ...
  if isSysInfo and self.curType == ChatType.Guild then --联盟聊天系统消息(不显示ICON)
    ...
    richText:setPositionX(imgBg:getPositionX() + imgBg:getContentSize().width - realSize.width/2)
  else 
    ...
    local lbUsrTitle = rootNode:getChildByName("Text_usrTitle") 
    ...
    if isSysInfo then 
      ...
      --阵营和分区
      local str = ""
      if self.curType == ChatType.Camp then 
        str = str .. "【"..g_tr("city_battle_camp"..g_PlayerMode.GetData().camp_id).."】"        
      elseif self.curType == ChatType.CityBattle then 
        if itemData.memo and itemData.memo.camp_id then 
          str = str .. "【"..g_tr("city_battle_camp"..itemData.memo.camp_id).."】"   
        end 
      end 
      if itemData.server_id then 
        str = str .. " S"..itemData.server_id.." "
      end 

      --联盟名称
      str = str ..(itemData.guild_short_name or "")
      local userTitle = ""
      if itemData.user_title then 
        for k, v in pairs(g_data.country_battle_title) do 
          if v.rank == itemData.user_title then 
            userTitle = g_tr(v.title_name)
          end 
        end 
      end 
      lbRank:setString(str or "")
      lbName:setString(itemData.nick)
      lbUsrTitle:setString(userTitle)
    ...
    lbUsrTitle:setPositionX(lbName:getPositionX()+lbName:getContentSize().width+20)
...
function ChatLayer:showChatList(chatType)
  ...
  for i = #chatData, 1, -1 do 
    ...
    if not ChatMode.isInBlackListEx(blackData, tmp.player_id) then 
      ...
      elseif chatType == self.typeOfBattle or chatType == ChatType.Camp then 
        table.insert(self.data, 1, tmp)
      end
    end
  ...
  if g_chatData.hasData(self.curType) then 
    ...
    local function onRecvAllData(result)
      ...
      if preTime ~= g_chatData.getCurChatDataTime(self.curType) then --有新数据
        ...
        if (self.curType == ChatType.Guild and g_AllianceMode.getSelfHaveAlliance()) 
          or self.curType == ChatType.World or self.curType == ChatType.Camp then 
          self:showChatList(self.curType)
        end 
      end 
    ...
    --战场/城战只允许在线推送,不支持向服务器拉数据
    if self.curType ~= self.typeOfBattle then 
...
function ChatLayer:initVoiceSpeeking() 
  print("initVoiceSpeeking")

  local function isUIReady()
    return self.nodeSend:getChildByName("Panel_voice"):isVisible()
  end 

  local function updateSentState(send_flag, para) 
    print("updateSentState:, flag = ", send_flag)

    if nil == layerObj then return end 

    if send_flag == SendFlag.PreSend then --UI列表插入语音项, 显示等待动画
      if para then         
        self:insertChatItem(para, -1) 
        self.listView:doLayout()
        self.listView:jumpToBottom() 
      end 
    else 
        ...
        if lbTag:getString() == filename then 
          ...
          if icon_fail:isVisible() then --支持点击重发
            self:regBtnCallback(icon_fail, function()
                print("start resend...") 
                ChatMode.sendVoiceMsg(g_chatData.getVoiceDataItem(self.typeOfBattle, filename), onSendResult, self.typeOfBattle) 
                g_chatData.updateVoiceDataSendFlag(self.typeOfBattle, filename, SendFlag.Waiting)
  ...
  local icon = nodeVoice:getChildByName("Image_3")
  local node_anim = nodeVoice:getChildByName("Panel_anim")
  ChatMode.registePressSpeeking(icon, node_anim, isUIReady, updateSentState) 
end 
...
function ChatLayer:onChatBattle()
  ...
  self:highlightMenu(self.typeOfBattle)

  if self.curType == self.typeOfBattle then return end 
  self.curType = self.typeOfBattle

  self:stopFrameLoad()
  self.listView:removeAllChildren()

  self:showChatList(self.typeOfBattle)
end 

function ChatLayer:onChatCamp()
  RecorderHelper.stopAudioPlay()
  g_musicManager.playEffect(g_SOUNDS_SURE_PATH)

  self:highlightMenu(ChatType.Camp)

  if self.curType == ChatType.Camp then return end 
  self.curType = ChatType.Camp
  self:stopFrameLoad()
  self:showChatList(ChatType.Camp) 
end 
...
function ChatLayer:onSendText()
  ...
  if self.curType == ChatType.World or self.curType == ChatType.Camp then 
    if g_PlayerMode.GetData().level < 10 then --10级限制
      local str = self.curType == ChatType.World and "chat_low_level" or "chat_low_level2"
      g_airBox.show(g_tr(str, {lv = 10})) 
      return 
    end 
  end 
  ...
  local function onSendSuccess(target, msgid, data)
    ...
    if data and data.flag then 
      ...
      elseif data.flag == "no_cross_battle" then 
        g_airBox.show(g_tr("chat_no_cross_battle")) 

      elseif data.flag == "not_join_cross_battle" then 
        g_airBox.show(g_tr("chat_not_join_cross_battle")) 

      end 
    end 
  end 

  print("self.curType, content", self.curType, contentStr)
...
function ChatLayer:updateChatData(dataItem)
  ...
  if self.curType == _type then --更新UI 
    if not ChatMode.isInBlackList(dataItem.player_id) then 
      ...
      elseif self.curType == self.typeOfBattle then
        needShow = true 

      elseif self.curType == ChatType.Camp then
        needShow = true         
      end
...
function ChatLayer:shareToGuild(name, x, y)
  ...
  ChatMode.sendChatMsg(ChatType.Guild, {player_id = myId, content = str}, nil)
end 
