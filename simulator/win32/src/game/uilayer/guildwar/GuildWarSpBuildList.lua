...
function GuildWarSpBuildList:updateView()
	...
	local spBuildDatas = g_guildWarMapSpBuildData.getSpBuildMapDatas()
	local spBuildPlayerDatas = g_guildWarMapSpBuildData.getSpBuildPlayerDatas()
	for key, var in pairs(spBuildDatas) do
		if var.area == self._areaId and var.map_element_origin_id == self._orginal_id then
			...
			local homeBigTileIndex = g_guildWarPlayerData.GetPosition()
			...
			btn:addClickEventListener(function()
				require("game.mapguildwar.changeMapScene").gotoWorldAndOpenInterface_BigTileIndex(cc.p(tonumber(var.x),tonumber(var.y)),function()
			...
			if var.player_id == 0 then
				...
				if playerData then
					item:getChildByName("Text_2"):setString(playerData.nick)
