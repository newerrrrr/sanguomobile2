...
function MailBaseLayer:initBinding(scaleNode)
  ...
  self.btnAddMember = self.nodeMailList:getChildByName("img_addMenbers")
  ...
  self:regBtnCallback(self.btnAddMember, onAddMenbers)
...
function MailBaseLayer:showMailTypeList(viewType)
  self.typeArray = {
      {MailHelper.viewType.System, "system"},               {MailHelper.viewType.SpyReport, "spyReport"},
      {MailHelper.viewType.BattleReport, "battleReport"},   {MailHelper.viewType.CollectionReport, "collectionReport"}, 
      {MailHelper.viewType.MonsterReport, "monsterReport"}, {MailHelper.viewType.Alliance, "alliance"}, 
      {MailHelper.viewType.ChatInfo, "chatInfo"},           {MailHelper.viewType.SendMail, "sendMail"}
    }
...
function MailBaseLayer:hightlightMailTypeIdx(idx)
  ...
  MailHelper:readyOnePageData(viewType) 
end 
...
function MailBaseLayer:showMailListByType(viewType, mailData, battleKind) 
  ...
  self.btnAddMember:setVisible(false)
...
function MailBaseLayer:showMailContent(viewType, listItem) 
  ...
  elseif viewType == MailHelper.viewType.ChatInfo then 
    ...
    --多人聊天时才显示添加成员按钮
    if listItem:getData().mail.type == MailType.ChatGroup then 
      self.btnAddMember:setVisible(true)
    end 
...
function MailBaseLayer:onAddMenbers()
  print("onAddMenbers") 
  g_musicManager.playEffect(g_SOUNDS_SURE_PATH)
end 
...
function MailBaseLayer:setMailIsRead(viewType, mailItem)
  ...
  --异步请求 
  g_sgHttp.postData("Mail/setRead",{mailIds = ids, type = viewType}, setReadResult, true)   
end 
...
function MailBaseLayer:insertNewMail(data)
  ...
  local viewType = self.typeArray[self.typeListIdx+1][1]
  local mailData = MailHelper:getListDataByMaxId(viewType, math.max(0, data.mail_id-1), false)
  dump(mailData, "====mailData")

  if data.cata_type == self.typeListIdx then  
    local mail 
    for i=1, #mailData do 
      local found = false 
      --更新列表,由于聊天支持叠加, 其他情况为单封邮件,只能插在最前面
