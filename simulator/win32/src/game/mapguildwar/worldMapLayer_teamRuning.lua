...
local TeamDisplayMD = require "game.mapguildwar.worldMapLayer_teamDisplay"
...
local function _playDynamicAttack(buildServerData, isWin, vec_back_attack)
	local tileData = require("game.mapguildwar.worldMapLayer_bigMap").getTileData_bigTileIndex(cc.p(buildServerData.x,buildServerData.y))
	if tileData then
...
function create_with_queueServerData(queueServerData, positionData)
	...
	--保存自己的公会ID
	local myGuildID = g_guildWarPlayerData.getGuildId()
	...
	local function onRecvBattleStatus(result, msgData)
		...
		if(result==true)then
			...
			or battleStatus == g_guildWarBattleInfoData.StatusType.STATUS_ATTACK_READY 
			or battleStatus == g_guildWarBattleInfoData.StatusType.STATUS_DEFEND_READY
			then
			else
				g_guildWarBattleInfoData.RequestData()
			end 
		end
	end
	local function sendForBattle()
	  ...
	  --比赛结算或者已结束 直接return
		...
		or battleStatus == g_guildWarBattleInfoData.StatusType.STATUS_ATTACK_CLAC 
		or battleStatus == g_guildWarBattleInfoData.StatusType.STATUS_DEFEND_CLAC
		or battleStatus == g_guildWarBattleInfoData.StatusType.STATUS_ATTACK_READY 
		or battleStatus == g_guildWarBattleInfoData.StatusType.STATUS_DEFEND_READY
		then
		...
		g_sgHttp.postData("cross/queueBattleRet",{ queueId = queueServerData.id },onRecvBattleStatus,true)
	end
	...
	local function updateArrival(current_time_real , current_time_unreal)
		if current_time_unreal >= timeData.endTime then
			...
			if display.lua_playAttack then
				--可能需要播放攻击动画
				local targetBuildServerData = require("game.mapguildwar.worldMapLayer_bigMap").getBuildServerData_originBigTileIndex(cc.p(queueServerData.to_x,queueServerData.to_y))
	...
	do --触摸部分
		...
		local function onTouchEnded(touch, event)
			if retRootLayer:isVisible() and cc.rectContainsPoint(cc.rect(0,0,c_ContentSize.width,c_ContentSize.height),cTools_worldToNodeSpace_position(retRootLayer,touch:getLocation())) then
				require "game.mapguildwar.worldMapLayer_bigMap".onClickTeam_queueServerData(queueServerData)
			end
		end
