...
local function _createResFlag(serverData, configData, originBigTileIndex)
	local filename = nil
	if serverData.player_id == g_guildWarPlayerData.GetData().player_id then
		--自己
		filename = "worldmap_image_flag_self.png"
	elseif serverData.guild_id ~= 0 and serverData.guild_id == g_guildWarPlayerData.getGuildId() then
		--盟友
...
function create_fort_range(serverData, configData, tp)
	...
	local col = ((serverData.guild_id ~= 0 and serverData.guild_id == g_guildWarPlayerData.getGuildId()) and c_range_color_self or c_range_color_other)
	if serverData.map_element_origin_id == HelperMD.m_MapOriginType.guild_war_toushiche then
		col = g_guildWarBattleInfoData.IsSelfOccupationArea(tonumber(serverData.area)) and c_range_color_self or c_range_color_other
	end
...
function create_gongchengchui(serverData, configData, originBigTileIndex)
	...
	local spBuildData = g_guildWarMapSpBuildData.getLocalSpBuildDataBy_xy(serverData.x,serverData.y)
	...
	armature , animation = g_gameTools.LoadCocosAni("anime/chongche/chongche.ExportJson", "chongche", onMovementEventCallFunc)
...
--	ret:runAction(action)
	...
	local function nodeEventHandler(eventType)
				...
				if data.Data.type == "hammerAttackDoor" then
					if spBuildData and spBuildData.build_display then
						...
						spBuildData.build_display.attackFunc = function()
							require "game.mapguildwar.worldMapLayer_bigMap".play_reduce_effect(num,mapConfigData,cc.p(targetX,targetY))
						end
		...
		elseif eventType == "exit" then
			g_gameCommon.removeEventHandler(g_Consts.CustomEvent.GuildWarMapEvent,ret)
			if spBuildData then
...
function create_chuangnu(serverData, configData, originBigTileIndex)
	...
	armature , animation = g_gameTools.LoadCocosAni("anime/nuche/nuche.ExportJson", "nuche", onMovementEventCallFunc)
--	local size = self:getContentSize()
--	armature:setPosition(cc.p(size.width / 2, size.height / 2))
	ret:addChild(armature)
...
function create_yunti(serverData, configData, originBigTileIndex)
	...
	armature , animation = g_gameTools.LoadCocosAni("anime/yunche/yunche.ExportJson", "yunche", onMovementEventCallFunc)
--	local size = self:getContentSize()
--	armature:setPosition(cc.p(size.width / 2, size.height / 2))
	ret:addChild(armature)
				...
				if data.Data.type == "catapultAttack" or data.Data.type == "catapultCounterAttack" then
					if spBuildData and spBuildData.build_display then
						...
						if spBuildData.x == fromX and spBuildData.y == fromY then
							...
							spBuildData.build_display.attackFunc = function()
								require "game.mapguildwar.worldMapLayer_bigMap".play_reduce_effect(num,mapConfigData,cc.p(targetX,targetY),configData)
							end
							local armature = spBuildData.build_display.armature 
							local animation = spBuildData.build_display.animation 
							if animation then
