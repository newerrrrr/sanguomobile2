	...
	if serverData.player_id ~= 0 then
		...
		if playerData.player_id == serverData.player_id then
			...
			if serverData.guild_id ~= 0 then
				if g_guildWarPlayerData.getGuildId() == serverData.guild_id then
					--自己公会（盟友）
					color = (QueueHelperMD.isGatherType(serverData) and not QueueHelperMD.isGatherReturnType(serverData)) and 5 or 3
				else
					--别人公会
					if to_map_data and ((to_map_data.guild_id ~= 0 and to_map_data.guild_id == g_guildWarPlayerData.getGuildId()) or to_map_data.player_id == playerData.player_id) then
						color = 4
