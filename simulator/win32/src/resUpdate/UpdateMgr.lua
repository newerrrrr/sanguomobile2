...
--延迟回调定时器(防止线程堵塞导致UI无法及时刷新)
function performDelayFrame(callback)

  local scheduler = cc.Director:getInstance():getScheduler()
  local delayTimer  
  local function doCallback()
    scheduler:unscheduleScriptEntry(delayTimer) 
  ...
  delayTimer = scheduler:scheduleScriptFunc(doCallback, 0, false)
end 
...
function onSuccess(customId)
  ...
  elseif customId == ID_BATCH then --批量更新结束
    ...
    elseif hasAssetToDownload() then --中间有下载失败时 
      release_print("has unfinish download, now retry...")
      retryDownload() 

    else --开始解压
      _viewUI:setTipStr(lang["decompresing"])
      
      performDelayFrame(function()
            if decompressZipFiles() and not hasUnzipError()  then 
    ...
    if not supportBatchDownload then --单线程下载时,继续下载下一个文件 
      downloadAssets() 
    end 
  end 

  if ret and not supportBatchDownload then 
    retryCount = 0 
  end 
end 
...
--配置里有两个开关来控制更新
--updateEnabled 控制是否更新
--skipCmpBigVer 允许更新时是否忽略大版本号(有时需要不同安装包共用一套资源)
...
function parseManifest() 
  ...
  remote_manifest = nil 
  --解压缩从服务器下载完的最新资源列表 
  --注意: 大小为0字节的zip文件,默认会解压缩成功, fuck !!!,所以需要校验文件的完整性
  if _fileUtils:isFileExist(path_manifest_zip) then 
    local newVerFile = loadJsonFile(path_version) 
    if newVerFile then 
      local size = _fileUtils:getFileSize(path_manifest_zip) 
      if size == newVerFile[manifest_name..".zip"].fileSize then 
        ...
        release_print("parseManifest: wrong manifest file size !!!!", size)
      end 
    end 
  else 
...
function redownloadFileSizeError()
  ...
  for k, v in pairs(download_list.fileSizeError) do 
    ...
    if _fileUtils:getFileSize(fullpath) ~= v[2].fileSize then 
      table.insert(download_list.assets, 1, v) 
    end 
  end 
...
function compareVersion(strVer1, strVer2) 
  ...
  if server_cfg and server_cfg.skipCmpBigVer == "true" then --忽略大版本号比较
    ret1 = 0 
  else 
...
function getLocalPkgVersion() 
  if nil == localPkgManifest then 
    localPkgManifest = loadJsonFile(localPath..manifest_name, true) 
  end 
