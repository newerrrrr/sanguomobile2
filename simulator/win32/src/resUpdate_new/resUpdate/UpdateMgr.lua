...
function initManifest()
  ...
  local_manifest = loadJsonFile(localPath..manifest_name, true) --apk包里的本地资源列表
...
function updateWhenSuccessOne(fileName, bDecompress)
  local item 
...
--   release_print("@@@ download apk")
--   _state = updateState.DownloadVersion
--   local url = getUrl(server_cfg.updateUrl, "test.apk", _state)
--   _dloadImpl:downLoadFileAsync(url, storagePath.."test.apk", "test.apk")  
-- end 

-- function parseVersion()
--   release_print("@@@ end apk")
--   local luaj = require("src.cocos.cocos2d.luaj")
--   local className="com/sanguomobile2/app/android/tw/MainActivity"
--   local params={storagePath.."test.apk"}
--   local arg="(Ljava/lang/String;)V"
--   luaj.callStaticMethod(className, "installApk", params, arg)
-- end 
...
function downloadAssets()
  ...
  if hasAssetToDownload() then 
    ...
    if supportBatchDownload then --多线程下载
      ...
      release_print("start to batch download")
      ...
      release_print("start to download:", info.fileName) 
    end 
  else 
