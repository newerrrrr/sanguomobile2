...
void SimulatorWin::destroyInstance()
{
	if (_instance)
		delete _instance;
	_instance = nullptr;
...
int SimulatorWin::run()
{
	...
	bool isOpenAutoUpdate = false;
	parseCocosProjectConfig(_project, &isOpenAutoUpdate);
	...
	_app->_isOpenAutoUpdate = isOpenAutoUpdate;
...
void SimulatorWin::parseCocosProjectConfig(ProjectConfig &config, bool * out_isOpenAutoUpdate)
{
	...
	if (out_isOpenAutoUpdate)
		*out_isOpenAutoUpdate = parser->isAutoUptate();
