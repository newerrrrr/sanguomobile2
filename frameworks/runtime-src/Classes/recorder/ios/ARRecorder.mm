...
#import "../AmrPcmConvert/VoiceConverter.h"
...
+ (void) stopRecord:(NSDictionary *)dict
{
    ...
    NSString *path_wav = [[ARRecorder getInstance].recordingPath stringByAppendingString: @".wav"];
    NSString *path_amr = [[ARRecorder getInstance].recordingPath stringByAppendingString: @".amr"];
    BOOL ret = [VoiceConverter wavToAmr:path_wav amrSavePath:path_amr];
...
+ (void) startPlaySound:(NSDictionary *)dict
{
    ...
    if([[path pathExtension] hasSuffix:@"amr"]) //将 amr 转换成 wav 播放
    {
        ...
        BOOL ret = [VoiceConverter amrToWav:path wavSavePath:path_wav];
        if (!ret)
        {
