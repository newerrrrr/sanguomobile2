...
void ConfigParser::readConfig(const string &filepath)
{
    ...
    if (_docRootjson.HasMember("init_cfg"))
    {
        if(_docRootjson["init_cfg"].IsObject())
        {
			...
			if (objectInitView.HasMember("isAutoUptate") && objectInitView["isAutoUptate"].IsBool())
			{
				_isAutoUptate = objectInitView["isAutoUptate"].GetBool();
			}
        }
    }
...
bool ConfigParser::isAutoUptate()
{
	return _isAutoUptate;
}
