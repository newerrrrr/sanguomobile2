...
def create_project_manifest():
    ...
    save_manifest(path_diff_res + "/project.manifest", project) #资源服务器放一份
    zip_files(path_diff_res + "/project.manifest.zip", path_diff_res + "/project.manifest", True) #压缩成zip
    GetFileMd5(path_diff_res + "/project.manifest.zip", version, None) #zip文件信息保存到version配置表中,供下载配置表时单独校验	
    save_manifest(path_diff_res + "/version.manifest", version) 
