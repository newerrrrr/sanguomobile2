...
function createHomeDistanceArrow(center,radius_y,radius_x)
	...
	arrow.lua_arrowUpdate = function( currentBigTileIndex , homeBigTileIndex , currentPositionCenter )
		...
		local world_position = require("game.maplayer.worldMapLayer_bigMap").position_2_worldPosition( cc.p(positionCenter.x, positionCenter.y + m_SingleSizeHalf.height) )
		local visibleRect = cc.rect(g_display.left, g_display.bottom, g_display.visibleSize.width, g_display.visibleSize.height)
		if world_position and cc.rectContainsPoint(visibleRect, world_position) == false then
	...
	local function onTouchEnded(touch, event)
		if arrow:isVisible() and cc.rectContainsPoint(rect,arrow:convertToNodeSpace(touch:getLocation())) == true then
			...
			bigMap.changeBigTileIndex_Manual(cc.p(playerData.x,playerData.y),true)
		end
	end
...
function createLoadingImage()
	...
	--loadingImage:setPosition(cc.p(g_display.left + (143 * g_display.scale), g_display.bottom + (200 * g_display.scale)))
