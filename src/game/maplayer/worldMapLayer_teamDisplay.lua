...
local TeamRoleMD = require "game.maplayer.worldMapLayer_teamRole"
...
function createDisplay_Troops(queueServerData, beginPosition, endPosition)
	...
	local myPlayerData = g_PlayerMode.GetData()
...
--[[	queueServerData.army_type = {}
	...
	if QueueHelperMD.isGatherGotoType(queueServerData)
		...
		for k , v in ipairs(teamTab) do
			...
			if queueServerData.player_id == myPlayerData.id then
	...
	if queueServerData.player_id == myPlayerData.id 
