...
local function isMySelf(buildServerData)
	return buildServerData.player_id ~= 0 and buildServerData.player_id == m_MyPlayerData.id
end


--是否属于自己联盟
local function isSelfGuild(buildServerData)
	return buildServerData.guild_id ~= 0 and buildServerData.guild_id == g_AllianceMode.getGuildId()
end
...
local function isHaveSelfQueueDoing(buildServerData,queueType)
	...
	for k , v in pairs(currentQueueDatas.Queue) do
		assert(v.to_map_id ~= 0, "error : to_map_id == 0 ")
		if buildServerData.id == v.to_map_id then
			if v.player_id == m_MyPlayerData.id and v.type == queueType then
				return v
			end
		end
	end
...
local function isHaveSelfGuildQueueDoing(buildServerData,queueType)
	if g_AllianceMode.getGuildId() ~= 0 then
		...
		for k , v in pairs(currentQueueDatas.Queue) do
			assert(v.to_map_id ~= 0, "error : to_map_id == 0 ")
			if buildServerData.id == v.to_map_id then
				if v.player_id ~= m_MyPlayerData.id and v.guild_id == g_AllianceMode.getGuildId() and v.type == queueType then
					return v
				end
			end
		end
...
local function isHaveOtherQueueDoing(buildServerData,queueType)
	...
	for k , v in pairs(currentQueueDatas.Queue) do
		assert(v.to_map_id ~= 0, "error : to_map_id == 0 ")
		if buildServerData.id == v.to_map_id then
			if v.player_id ~= m_MyPlayerData.id and  (v.guild_id == 0 or v.guild_id ~= g_AllianceMode.getGuildId())  and v.type == queueType then
				return v
			end
		end
	end
...
function create_with_buildServerData(buildServerData , tipMenuId)
	...
	local function widgetEventHandler(eventType)
        if eventType == "enter" then
			if buildServerData.map_element_origin_id == HelperMD.m_MapOriginType.guild_tower then
	...
	elseif map_element_origin_id == HelperMD.m_MapOriginType.player_home then--------------------------------------------------------------城堡
		...
		elseif isSelfGuild(buildServerData) then
			--自己联盟的其他玩家城堡
			local queueSD = require "game.maplayer.worldMapLayer_bigMap".getSelfQueueDoing_bigTileIndex_queueType(buildServerData.id, QueueHelperMD.QueueTypes.TYPE_CITYASSIST_ING)
			if queueSD then
				--自己在援助
				menu_Key = 5
			else
	...
	local function onCellButton(sender, eventType)
		...
		elseif eventType == ccui.TouchEventType.ended then
			if open_animation_completed then
				require("game.maplayer.worldMapLayer_smallMenuClick").onClick(
		...
		if map_element_origin_id == HelperMD.m_MapOriginType.player_home and isMySelf(buildServerData) then
			...
			local function onSaveIndexButton(sender, eventType)
				if eventType == ccui.TouchEventType.ended then
					require("game.maplayer.worldMapLayer_pecialClick").onClick_SaveIndex(cc.p(buildServerData.x,buildServerData.y), g_data.map_element[tonumber(buildServerData.map_element_id)], buildServerData)
					bigMap.closeSmallMenu()
				end
			end
...
local function isMySelf_queue(queueServerData)
	return queueServerData.player_id ~= 0 and queueServerData.player_id == m_MyPlayerData.id
end
...
local function isSelfHaveQueueInQueue_queue(queueServerData)
	...
	local currentQueueDatas = bigMap.getCurrentQueueDatas()
	for k , v in pairs(currentQueueDatas.Queue) do
		if v.player_id == m_MyPlayerData.id and v.parent_queue_id == queueServerData.id then
			return v
		end
	end
	return nil
...
function create_with_bigTileIndex(bigTileIndex , tipMenuId)
	...
	--最终需要的key
	local menu_Key = g_AllianceMode.isAllianceManager() and 2 or 1
	...
	do
		...
		local function onSaveIndexButton(sender, eventType)
			if eventType == ccui.TouchEventType.ended then
				require("game.maplayer.worldMapLayer_pecialClick").onClick_SaveIndex(bigTileIndex, nil, nil)
				bigMap.closeSmallMenu()
			end
		end
		bottom:getChildByName("Image_4"):addTouchEventListener(onSaveIndexButton)
...
function setTipMenuID( tipMenuId )
	local widget = require "game.maplayer.worldMapLayer_bigMap".getSmallMenu()
	if widget then
