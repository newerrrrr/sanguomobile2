...
local AreaMapMD = require "game.mapguildwar.worldMapLayer_areaMap"
local QueueHelperMD = require "game.mapguildwar.worldMapLayer_queueHelper"
local LineMD = require "game.mapguildwar.worldMapLayer_line"
local SmallMenuMD = require "game.mapguildwar.worldMapLayer_smallMenu"
local TeamRuningMD = require "game.mapguildwar.worldMapLayer_teamRuning"
local InputMenuMD = require "game.mapguildwar.worldMapLayer_inputMenu"
local TouchMaskMD = require "game.mapguildwar.worldMapLayer_touchMask"
...
local BuildTitleMD = require "game.mapguildwar.worldMapLayer_buildTitle"
local BuildEffectMD = require "game.mapguildwar.worldMapLayer_buildEffect"
local BuildDisplayMD = require "game.mapguildwar.worldMapLayer_buildDisplay"
...
local isForceShowFog = true --是否强制显示迷雾(只有在isMapTest 为 true时生效)
	...
	local function rootLayerEventHandler(eventType)
		if eventType == "enter" then
			...
			if isMapTest == true then
				schedulers[(#schedulers) + 1] = cc.Director:getInstance():getScheduler():scheduleScriptFunc(require("game.mapguildwar.worldMapLayer_uiLayer").onMapUpdate, 8 , false)
			end
...
--	rootLayer:addChild(m_HomeArrow,3)
	...
	--地图底层
	local tmxLayer = ccexp.TMXTiledMap:create("worldmap/mapRes/zuob1.tmx")
	...
	do
		local function onScrollViewDidScroll()
			...
			--m_HomeArrow.lua_arrowUpdate(bigTileIndex , g_guildWarPlayerData.GetPosition() , lookCenterPosition)
			require("game.mapguildwar.worldMapLayer_uiLayer").updateShow_bigTileIndex(bigTileIndex)
			require("game.mapguildwar.worldMapLayer_uiLayer").updateShow_arrow(bigTileIndex , g_guildWarPlayerData.GetPosition() , lookCenterPosition)
		end
	...
	--g_guildWarGuildPlayersData.RequestData()
	...
	--如果没有选择复活点，自动弹出选择界面
	if tonumber(g_guildWarPlayerData.GetData().is_in_map) == 0 and require("game.mapguildwar.worldMapLayer_bigMap").isMapTest ~= true then
...
local function _requestMapAllData(isAsync)
		...
		if isAsync then
			...
			if battleStatus == g_guildWarBattleInfoData.StatusType.STATUS_FINISH then
				return 
			end
		end
...
--		end
			...
			--优化过这里不用发送area list
--		local areaList = g_guildWarBattleInfoData.GetCurrentArea()
--		--for test
--		areaList = {1,2,3,4,5}

		g_sgHttp.postData("Cross/showBlockNQueue", { queueList = {centerAreaID, } }, onRecvShowBlockNQueue, isAsync) --请求地图
...
function onMsgMapAllData(msgData)
	...
	local crossPlayerData = msgData.crossPlayer
	
	do --优先给spbuild缓存 因为建筑创建和更新可能需要依赖最新的spbuild
		if spBuildMsgData then
			m_CurrentSpBuildDatas = spBuildMsgData
		end
		g_guildWarMapSpBuildData.setSpBuildData(m_CurrentSpBuildDatas)
	end
	...
	do--处理地图块消息
		if mapMsgData then
			...
			for k , v in pairs(mapMsgData.Map) do
				...
				and v.guild_id == g_guildWarPlayerData.getGuildId() then
	...
	g_guildWarBattleInfoData.SetData(battleInfoMsgData)
	g_guildWarBattleInfoData.SetTopPlayerData(battleInfoTopPlayerData)
	g_guildWarBattleInfoData.NotificationUpdateShow()
	
	if crossPlayerData then
		g_guildWarPlayerData.SetData(crossPlayerData)
		g_guildWarPlayerData.NotificationUpdateShow()
	end
	
	
	require("game.mapguildwar.worldMapLayer_uiLayer").onMapUpdate(msgData)
...
function getSelfQueueDoing_bigTileIndex_queueType(to_map_id, queueType)
	local myPlayerID = g_guildWarPlayerData.GetData().player_id
...
function update_worldMap_2(dt)
	...
	--g_guildWarPlayerData.RequestDataAsync()
end
...
--	local battleStatus = g_guildWarBattleInfoData.GetData().status
--	if battleStatus ~= g_guildWarBattleInfoData.StatusType.STATUS_FINISH then
--		g_guildWarGuildPlayersData.RequestDataAsync()
--	end
end
...
function updateFog()
	...
	local areaList = clone(g_guildWarBattleInfoData.GetCurrentArea())
	if not g_guildWarBattleInfoData.IsAttacker() then
		local atArea1 = false
		local atArea2 = false
		
		for key, var in pairs(m_selfGuildPlayers) do
			if not atArea1 and tonumber(var.area) == 1 then
				atArea1 = true
			end
			
			if not atArea2 and tonumber(var.area) == 2 then
				atArea2 = true
			end
			
			if atArea1 and atArea2 then
				break
			end
		end
	...
	if isMapTest == true and isForceShowFog == true then
		for i = 1, 5 do  
			areaList[i] = i --强制全部不显示
		end
...
--		areaList[4] = nil
	...
	for i = 1, 5 do
		if areaList[i] == nil then
			if m_FogViewList["fog"..i] == nil then
				local fog = cc.CSLoader:createNode("fog_0"..i..".csb")
				local fogPos = fogPosList[i]
				local effectNode = cc.Node:create()
				effectNode:setContentSize(cc.size(1.0,1.0))
				effectNode:setPosition(HelperMD.bigTileIndex_2_positionCenter(fogPos))
				effectNode:addChild(fog)
				local size = fog:getChildByName("scale_node"):getContentSize()
				fog:setPosition(cc.p(-size.width/2,-size.height/2))
				m_FogNode:addChild(effectNode)
			  m_FogViewList["fog"..i] = effectNode
			end
		else
			if m_FogViewList["fog"..i] then
				m_FogViewList["fog"..i]:removeFromParent()
				m_FogViewList["fog"..i] = nil
			end
		end 
	end
...
function checkSingleBuild_msgData(serverData)
	...
	if serverData.map_element_origin_id == HelperMD.m_MapOriginType.guild_fort then
		...
		if range_data then
			local getNameFunc = ((serverData.guild_id ~= 0 and serverData.guild_id == g_guildWarPlayerData.getGuildId()) and HelperMD.getImageNmaeWithSideTypeSelf or HelperMD.getImageNmaeWithSideTypeOther)
...
--		local range_data = m_CurrentRanges[idString]
--		if range_data then
--			local getNameFunc = g_guildWarBattleInfoData.IsSelfOccupationArea(tonumber(serverData.area)) and HelperMD.getImageNmaeWithSideTypeSelf or HelperMD.getImageNmaeWithSideTypeOther
--			for k1 , v1 in pairs(range_data.areas) do
--				local tmxMap = m_BuildNode:getChildByTag(k1)
--				if tmxMap then
--					local tileLayer = tmxMap.lua_layer_mid_idArray[idString]
...
function addSingleBuildRange(serverData,configData,originBigTileIndex,ZOrder)
	...
	local getNameFunc = ((serverData.guild_id ~= 0 and serverData.guild_id == g_guildWarPlayerData.getGuildId() ) and HelperMD.getImageNmaeWithSideTypeSelf or HelperMD.getImageNmaeWithSideTypeOther)
--	if serverData.map_element_origin_id == HelperMD.m_MapOriginType.guild_war_toushiche then
--		getNameFunc = g_guildWarBattleInfoData.IsSelfOccupationArea(tonumber(serverData.area)) and HelperMD.getImageNmaeWithSideTypeSelf or HelperMD.getImageNmaeWithSideTypeOther
--	end
...
function addQueueNotice(serverData)
	...
	--驻守
	if serverData.player_id == myPlayerID or ( serverData.guild_id ~= 0 and serverData.guild_id == g_guildWarPlayerData.getGuildId() ) then
...
function onClickBuild_buildServerData(buildServerData)
	if buildServerData.map_element_origin_id == HelperMD.m_MapOriginType.monster_small then
		...
		--小怪没小菜单
		require("game.mapguildwar.worldMapLayer_pecialClick").onClick_SmallMonster(g_data.map_element[tonumber(buildServerData.map_element_id)], buildServerData)
		--mask
	...
	elseif buildServerData.map_element_origin_id == HelperMD.m_MapOriginType.monster_boss then
		g_musicManager.playEffect(g_data.sounds[5000038].sounds_path)
		--BOSS怪没小菜单
		require("game.mapguildwar.worldMapLayer_pecialClick").onClick_BossMonster(g_data.map_element[tonumber(buildServerData.map_element_id)], buildServerData)
		--mask
		m_TouchMaskNode:addChild(TouchMaskMD.create_NoSmallMenu(buildServerData,nil),1,c_tag_touchMask_noSmallMenu)
		return
	elseif buildServerData.map_element_origin_id == HelperMD.m_MapOriginType.heshibi then
		--和氏璧没小菜单
		require("game.mapguildwar.worldMapLayer_pecialClick").onClick_Heshibi(g_data.map_element[tonumber(buildServerData.map_element_id)], buildServerData)
		--mask
		m_TouchMaskNode:addChild(TouchMaskMD.create_NoSmallMenu(buildServerData,nil),1,c_tag_touchMask_noSmallMenu)
		return
