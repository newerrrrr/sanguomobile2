	...
	if g_guildWarPlayerData.getGuildId() ~= 0 then
		...
		for k , v in pairs(currentQueueDatas.Queue) do
			...
			if buildServerData.id == v.to_map_id then
				if v.guild_id == g_guildWarPlayerData.getGuildId() and v.type == queueType then
					return true
				end
			end
		end
...
local function _create_playHome(serverData, configData, originBigTileIndex,showDurability)
	...
	if showDurability == true then
		...
		local currentAreaDatas = require "game.mapguildwar.worldMapLayer_bigMap".getCurrentAreaDatas()
	...
	if serverData.guild_id ~= 0 then
		...
		color = g_guildWarPlayerData.getGuildId() == serverData.guild_id and cc.c4b(0,255,0,255) or cc.c4b(255,0,0,255)
	...
	if playerData then
		if tonumber(g_guildWarPlayerData.GetData().player_id) == tonumber(serverData.player_id) then
...
local function _create_guild(serverData, configData, originBigTileIndex)
	...
	if serverData.map_element_origin_id == HelperMD.m_MapOriginType.guild_fort then
		--联盟堡垒才有驻防
		if serverData.guild_id ~= 0 and g_guildWarPlayerData.getGuildId() == serverData.guild_id then
			--自己公会才看驻防信息
...
local function _create_camp(serverData, configData, originBigTileIndex,showDurability)
	...
	then
		...
		local cdTime = tonumber(g_data.warfare_service_config[19].data)
...
--		bg:setOpacity(100)
    ...
    node:addChild(textProgressNode)
   	...
   	if serverData.resource >= maxValue then
   		...
   		local showNormalStatus = function()
   			widget:getChildByName("Panel_6"):setVisible(g_guildWarBattleInfoData.IsAttacker())
   			cd:setVisible(true)
   			cd1:setVisible(false)
   			bg:setVisible(true)
   			bg1:setVisible(false)
   			...
   			if serverData.durability <= 0 then
				...
				local finishTime = require ("game.mapguildwar.worldMapLayer_queueHelper").getYunTiProgressTime(tonumber(serverData.x), tonumber(serverData.y), serverData.map_element_origin_id)
				if finishTime then
  ...
  then
  	...
  	local currentBuildIdx = g_guildWarMapSpBuildData.getLocalSpBuildDataBy_xy(serverData.x,serverData.y).build_num
		...
		local function rootLayerEventHandler(eventType)
			...
			elseif eventType == "exit" then
				g_gameCommon.removeEventHandler(g_Consts.CustomEvent.GuildWarMapEvent,noEnterCon)
	    end
	  end
  	...
  	if not g_guildWarBattleInfoData.IsAttacker() 
	...
	--		bg:setPosition(pos)
	   	...
	   	loadingBar:setPercent(0)
	   	--local cdTime = tonumber(g_data.warfare_service_config[14].data)
	   	--cd改为从服务器获取
  ...
  then
  	local playerData = require "game.mapguildwar.worldMapLayer_bigMap".getCurrentAreaDatas().Player[tostring(serverData.player_id)]
  	if playerData and tonumber(serverData.guild_id) == tonumber(g_guildWarPlayerData.getGuildId()) then
  		widget:getChildByName("Panel_6"):setVisible(true)
	...
	--		bg:setPosition(pos)
	   	...
	   	loadingBar:setPercent(0)
	   	local cdTime = tonumber(g_data.warfare_service_config[9].data)
	   	local finishTime = serverData.attack_time + cdTime
	   	if g_clock.getCurServerTime() > finishTime then
	   		loadingBar:setPercent(100)
	   		iconGetedLight:setVisible(true)
	...
	then
		...
		local cdTime = tonumber(g_data.warfare_service_config[36].data) --复活cd
		local nextRecoveTime = serverData.recover_time 
...
--		bg:setOpacity(100)
    ...
    node:addChild(textProgressNode)
		...
		local showNormalStatus = function()
			widget:getChildByName("Panel_6"):setVisible(g_guildWarBattleInfoData.IsAttacker() and serverData.guild_id ~= 0)
 			cd:setVisible(true)
 			cd1:setVisible(false)
 			bg:setVisible(true)
 			bg1:setVisible(false)
	   	...
	   	--local cdTime = tonumber(g_data.warfare_service_config[12].data) --攻击cd
	   	--cd改为从服务器获取
	   	local cdTime = serverData.attack_cd
	   	local finishTime = serverData.attack_time + cdTime
  ...
  and serverData.player_id == 0 then
  	...
  	local updateTitleShow = function()
	  	--投石车空闲时需要显示攻或者守
	  	if g_guildWarBattleInfoData.IsAttacker() then
	  		if g_guildWarBattleInfoData.IsSelfOccupationArea(serverData.area) then
