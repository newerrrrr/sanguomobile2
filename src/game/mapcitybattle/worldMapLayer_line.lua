	...
	if serverData.player_id ~= 0 then
		...
		if playerData.player_id == serverData.player_id then
			...
			if serverData.camp_id ~= 0 then
				if g_cityBattlePlayerData.getCampId() == serverData.camp_id then
					--自己公会（盟友）
					color = (QueueHelperMD.isGatherType(serverData) and not QueueHelperMD.isGatherReturnType(serverData)) and 5 or 3
				else
					--别人公会
					if to_map_data and ((to_map_data.camp_id ~= 0 and to_map_data.camp_id == g_cityBattlePlayerData.getCampId()) or to_map_data.player_id == playerData.player_id) then
						color = 4
					elseif serverData.to_x == playerData.x and serverData.to_y == playerData.y then
						color = 4
					else
...
function create_with_queueServerData( queueServerData , positionData )
	...
	if colorType == 2 or colorType == 5 or colorType == 4 then
		isAboutSelfQueue = true
	end
	...
	clipNode.isAboutSelfQueue = _isAboutMy(queueServerData)
