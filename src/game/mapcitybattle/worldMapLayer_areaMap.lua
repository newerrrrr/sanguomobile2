...
local MapArrayMD = require "game.mapcitybattle.worldMapLayer_mapArray"
...
function create(areaIndex)
	...
	local tmxLayer = lhs.LHSTmxLayer:create(string.format("worldmap/mapRes/map_citybattle_area_%d.tmx",MapArrayMD.map[areaIndex.y + 1][areaIndex.x + 1]))
	m_Root = tmxLayer
	...
	if g_isDebug then
		if tmxLayer:getChildByName("layer_1") == nil 
			...
			or tmxLayer:getChildByName("layer_area") == nil
				then
