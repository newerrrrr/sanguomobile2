...
local TeamDisplayMD = require "game.mapcitybattle.worldMapLayer_teamDisplay"
...
local function findQueueMapElement(map_id)
	if map_id then
		local d = require("game.mapcitybattle.worldMapLayer_bigMap").getCurrentQueueDatas().MapElement[tostring(map_id)]
		if d then
	...
	elseif QueueHelperMD.isFetchItemType(queueServerData) then
		...
		elseif QueueHelperMD.isPlayCarriageType(queueServerData) then
			...
			if isStronghold then
				...
				display , offsetPosition = TeamDisplayMD.createDisplay_CityBattle(queueServerData, beginPosition, endPosition)
				ret:addChild( display )
				ret.lua_playAttack = display.lua_playAttack --播放打斗函数
...
local function _opIsNeedRequestTimeExpand(myPlayerID, myGuildID , queueServerData, targetBuildServerData)
	return (
			...
			or (myGuildID ~= 0 and queueServerData.camp_id == myGuildID)
...
local function _isAboutMy(serverData)
	
	local isAboutMy = false
	...
	if serverData.player_id ~= 0 then
		if playerData.player_id == serverData.player_id then
			--我
			isAboutMy = true
		else
			if to_map_data and to_map_data.player_id == playerData.player_id then
				isAboutMy = true
...
local function _playDynamicAttack(buildServerData, isWin, vec_back_attack)
	local tileData = require("game.mapcitybattle.worldMapLayer_bigMap").getTileData_bigTileIndex(cc.p(buildServerData.x,buildServerData.y))
	if tileData then
...
function create_with_queueServerData(queueServerData, positionData)
	...
	--保存自己的公会ID
	local myGuildID = g_cityBattlePlayerData.getCampId()
	...
	local function onRecvBattleStatus(result, msgData)
		...
		if(result==true)then
			...
			if battleStatus == g_cityBattleInfoData.StatusType.STATUS_ATTACK_CLAC 
			or battleStatus == g_cityBattleInfoData.StatusType.STATUS_DEFEND_CLAC
			or battleStatus == g_cityBattleInfoData.StatusType.STATUS_ATTACK_READY 
			or battleStatus == g_cityBattleInfoData.StatusType.STATUS_DEFEND_READY
			then
			else
				g_cityBattleInfoData.RequestData()
			end 
		end
	end
	local function sendForBattle()
	  ...
	  --比赛结算或者已结束 直接return
		local battleStatus = g_cityBattleInfoData.GetData().status
		if battleStatus == g_cityBattleInfoData.StatusType.STATUS_FINISH
		or battleStatus == g_cityBattleInfoData.StatusType.STATUS_ATTACK_CLAC 
		or battleStatus == g_cityBattleInfoData.StatusType.STATUS_DEFEND_CLAC
		or battleStatus == g_cityBattleInfoData.StatusType.STATUS_ATTACK_READY 
		or battleStatus == g_cityBattleInfoData.StatusType.STATUS_DEFEND_READY
		then
		...
		g_sgHttp.postData("City_Battle/queueBattleRet",{ queueId = queueServerData.id },onRecvBattleStatus,true)
	end
	...
	local function updateArrival(current_time_real , current_time_unreal)
		if current_time_unreal >= timeData.endTime then
			...
			if display.lua_playAttack then
				--可能需要播放攻击动画
				local targetBuildServerData = require("game.mapcitybattle.worldMapLayer_bigMap").getBuildServerData_originBigTileIndex(cc.p(queueServerData.to_x,queueServerData.to_y))
					...
					if battleStatusTab.postStatus == false then ---没有在请求中
						...
						elseif battleStatusTab.battleStatus == k_QueueBattleEnum.win then		------------------战斗胜利
							...
							if queueServerData.player_id == myPlayerID then 
								...
								local showBattleResultTip = true
								local to_cf = findQueueMapElement(queueServerData.to_map_id)
								if to_cf and to_cf.origin_id == HelperMD.m_MapOriginType.guild_war_gate then --攻打城门不提示结果
									showBattleResultTip = false
								end
								
								if showBattleResultTip then
									--自己在战斗给结果提示
	...
	do --触摸部分
		...
		local function onTouchEnded(touch, event)
			if retRootLayer:isVisible() and cc.rectContainsPoint(cc.rect(0,0,c_ContentSize.width,c_ContentSize.height),cTools_worldToNodeSpace_position(retRootLayer,touch:getLocation())) then
				require "game.mapcitybattle.worldMapLayer_bigMap".onClickTeam_queueServerData(queueServerData)
			end
		end
	...
	retRootLayer.isAboutSelfQueue = _isAboutMy(queueServerData)
	return retRootLayer
end
