...
m_MapOriginType = {
	...
	guild_war_gongchengchui = 402, --攻城锤
	
	guild_war_gate = 401,	--城门
	
	guild_war_chuangnu = 405,	--床弩
	
	guild_war_yunti = 403,	--云梯
	
	guild_war_toushiche = 404,	--投石车
...
function createHomeDistanceArrow(center,radius_y,radius_x)
	...
	arrow.lua_arrowUpdate = function( currentBigTileIndex , homeBigTileIndex , currentPositionCenter )
		...
		local world_position = require("game.mapcitybattle.worldMapLayer_bigMap").position_2_worldPosition( cc.p(positionCenter.x, positionCenter.y + m_SingleSizeHalf.height) )
		local visibleRect = cc.rect(g_display.left, g_display.bottom, g_display.visibleSize.width, g_display.visibleSize.height)
		if world_position and cc.rectContainsPoint(visibleRect, world_position) == false and g_cityBattlePlayerData.GetData().is_in_map == 1 then
