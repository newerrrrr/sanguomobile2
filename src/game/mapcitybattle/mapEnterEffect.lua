local mapEnterEffect = {}
setmetatable(mapEnterEffect,{__index = _G})
setfenv(1,mapEnterEffect)
...
function create(callback)
	...
	local animPath = "anime/Effect_NewZhanChangChengMenZhanGuoDu/Effect_NewZhanChangChengMenZhanGuoDu.ExportJson"
	local armature , animation = nil , nil
	local uiLayer = nil
	...
	armature , animation = g_gameTools.LoadCocosAni(
		animPath
		, "Effect_NewZhanChangChengMenZhanGuoDu"
		, onMovementEventCallFunc
		--, onFrameEventCallFunc
		)
	ret:addChild(armature)
	...
	if g_cityBattleInfoData.IsDoorMap() then
		if battleInfo.camp_id == 0 then
			attackList = {1,2,3}
		elseif battleInfo.camp_id == 1 then
			defensList = {battleInfo.camp_id}
			attackList = {2,3}
		elseif battleInfo.camp_id == 2 then
			defensList = {battleInfo.camp_id}
			attackList = {1,3}
		elseif battleInfo.camp_id == 3 then
			defensList = {battleInfo.camp_id}
			attackList = {1,2}
		end
	else
		attackList = {battleInfo.attack_camp}
		defensList = {battleInfo.defend_camp}
	end
	
	local attackBoneNames = {"QiZhi03","QiZhi02","QiZhi01"}
	local defensBoneNames = {"QiZhi04","QiZhi05","QiZhi06"}
	
	do
		for key, var in pairs(attackList) do
			if var > 0 then
				local container = cc.Node:create()
				container:setCascadeOpacityEnabled(true)
				local iconId = g_data.country_camp_list[var].camp_pic
			  uiLayer = g_resManager.getRes(iconId)
			  if uiLayer then
				  container:addChild(uiLayer)
					armature:getBone(attackBoneNames[key]):addDisplay(container,0)
				end
			end
		end
	end
	...
	do
		for key, var in pairs(defensList) do
			if var > 0 then
				local container = cc.Node:create()
				container:setCascadeOpacityEnabled(true)
				local iconId = g_data.country_camp_list[var].camp_pic
			  ...
			  if uiLayer then
				  container:addChild(uiLayer)
					armature:getBone(defensBoneNames[key]):addDisplay(container,0)
				end
			end
		end
	end
...
--	end
	...
	--[[if nameList == nil then
		...
		g_busyTip.show_1()
		g_sgHttp.postData("City_Battle/getAllPlayerList", {}, onRecv,true)
	else
		playAnima()
	end
	]]
