...
local MapEnterEffect = require("game.mapcitybattle.mapEnterEffect")
...
local function _goHome(place, gotoSucceedCallback)
	local function onChangeEvent(event)
		if event == CloudEffectMD.m_EnevtEnum.close_start then
			m_isChanging = true
			require("game.mapcitybattle.worldMapLayer_bigMap").playGoHome()
...
local function _goWorld(bigTileIndex, gotoSucceedCallback)
	...
	local function onChangeEvent(ret,armature)
		g_sceneManager.clearAllNodeForMap()
		g_sceneManager.addNodeForMap(require("game.mapcitybattle.worldMapLayer_bigMap").create(bigTileIndex,map_type))
						
		require("game.uilayer.mainSurface.mainSurfaceModeShow").mainScrfaceChangeView()
		require("game.mapcitybattle.worldMapLayer_bigMap").playFromHomeComeBack()
	...
	g_sceneManager.addNodeForTopEffect(MapEnterEffect.create(onChangeEvent))
...
local function _goWorldAndOpenInterface(bigTileIndex, gotoSucceedCallback)
	local function onChangeEvent(event)
		...
		elseif event == CloudEffectMD.m_EnevtEnum.open_start then
			require("game.mapcitybattle.worldMapLayer_bigMap").playFromHomeComeBack()
		elseif event == CloudEffectMD.m_EnevtEnum.open_complete then
			require("game.mapcitybattle.worldMapLayer_bigMap").changeBigTileIndexAndOpenInterface_Manual(bigTileIndex,true)
			m_isChanging = false
			g_musicManager.playMusic(g_data.sounds[5000003].sounds_path,true)
			if gotoSucceedCallback then
				gotoSucceedCallback()
...
function reloadMap(gotoSucceedCallback )
	_goWorld(g_cityBattlePlayerData.GetPosition(), gotoSucceedCallback)
end

function changeToWorld( isInit, gotoSucceedCallback )
	
	if isInit == true then
		m_MapStatus = m_MapEnum.citybattle
		ChangeMapScene.setCurrentMapStatus(m_MapStatus)
		
		local data = g_cityBattlePlayerData.GetData()
		_goWorldDirect(g_cityBattlePlayerData.GetPosition(), gotoSucceedCallback)
		
	else
		if ChangeMapScene.getCurrentMapStatus() == m_MapEnum.citybattle then
			if gotoSucceedCallback then
				gotoSucceedCallback()
			end
			return
...
--	if mapStatus == changeMapScene.m_MapEnum.citybattle or m_isChanging == true then
--	else
