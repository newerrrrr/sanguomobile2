local guildWarInfo = class("guildWarInfo", require("game.uilayer.base.BaseLayer"))

function guildWarInfo:ctor(x,y)
    guildWarInfo.super.ctor(self)
    self._x = x
    self._y = y
    self.warInfo = nil
end

function guildWarInfo:InitUI()
...
function guildWarInfo:onEnter()
    ...
    g_sgHttp.postData("cross/spy",{ x = self._x , y = self._y },onRecv)
end


function guildWarInfo:onExit()
