local cwBattleSettingView = class("cwBattleSettingView",require("game.uilayer.base.BaseLayer"))
...
--post:目的地的XY
--fightType:计算时间战斗类型
function cwBattleSettingView:createLayer( fun,postb,fightType)
    ...
    self.playerData = g_cityBattlePlayerData.GetData()

    g_sceneManager.addNodeForUI(cwBattleSettingView:create( fun ))

    return true
end

function cwBattleSettingView:ctor(fun)
    cwBattleSettingView.super.ctor(self)

    self.callback = fun
    self.armyList = {}
    g_groundData.RequestSycCityBattleBattleData( function (result,data)
        if result == true then
            self.playerArmyData = g_cityBattleArmy.GetData()
            self.playerArmyUnitData = g_cityBattleArmyUnit.GetData()
...
function cwBattleSettingView:onEnter()
...
function cwBattleSettingView:filterData(callback)
    ...
    g_busyTip.show_1()
    g_sgHttp.postData("City_Battle/getGotoTime", { x = self.postb.x,y = self.postb.y,type = self.fightType }, callback,true)

end


function cwBattleSettingView:itemInitUI(item,data)
    ...
    resTx:setString(g_tr("city_battle_full_soldier_cd"))
    
    local timeLabel = resNumTx
    ...
    do
    	...
    	local updateCd = function()
    		...
    		local finishTime = data.fill_soldier_time + cdTime
...
--			  end
    ...
    for i , gen_item in ipairs(gen_tb) do
        ...
        if d and d ~= 0 then
            ...
            if pic.icon == nil then
                local general = g_cityBattleGeneral.getOwnedGeneralByOriginalId(t.general_original_id)
            ...
            local max_soldier = g_cityBattleArmy.GetMaxArmyNum(d.general_id)
    ...
    self:regBtnCallback(btn_edit,function ()
        ...
        g_sceneManager.addNodeForUI(require("game.uilayer.drill.CityBattleDrillView").new(callback,tabType))
	end)
    ...
    self:regBtnCallback( view_btn,function ()
        ...
        require("game.mapcitybattle.changeMapScene").changeToWorld( false, function ()
            local queueServerData = require("game.mapcitybattle.worldMapLayer_bigMap").getQueueServerData_armyId(ArmyID)
    ...
    resNumTx:setString( tostring(g_cityBattleSoldier.GetAllSoldierNumber()) )
    ...
    local function oneKey(isShow)
        ...
        g_busyTip.show_1()
        g_netCommand.send("City_Battle/fullfillSoldier", { ["armyPosition"] = data.position }, callback,true)
    end
    ...
    self:regBtnCallback( addRyBtn,function ()
        ...
        mode:buyCityBattleSoldier(2,oneKey)--贡献
        resNumTx:setString( tostring(g_cityBattleSoldier.GetAllSoldierNumber()) )
    end)
    
    addRyBtn:setVisible(false)
    ...
    self:regBtnCallback( addYbBtn,function ()
        ...
        local mode = require("game.uilayer.drill.DrillMode").new()
        mode:buyCityBattleSoldier(1,oneKey)--元宝
        resNumTx:setString( tostring(g_cityBattleSoldier.GetAllSoldierNumber()) )
    end)
		
		addYbBtn:setVisible(false)
...
function cwBattleSettingView:initUI()
...
function cwBattleSettingView:onExit()
    self:clearGlobal()
end


function cwBattleSettingView:clearGlobal()
...
--武将所带的兵数量未达到最高 提示
function cwBattleSettingView:confirm(ArmyID,tabType,_callback)
...
function cwBattleSettingView:noArmyConfirm()
    --g_msgBox.show( g_tr("battleOutIsEmpty"))
    g_airBox.show(g_tr("battleGuildAir"))
end

function cwBattleSettingView:addCallBack(fun)
    self.parCallBack = fun
end

function cwBattleSettingView:goOut(ArmyID,outType)
...
function cwBattleSettingView:setUsePowerType(type)
    self.usePower = 0
