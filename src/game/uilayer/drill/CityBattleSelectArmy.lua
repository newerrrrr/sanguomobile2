local CityBattleSelectArmy = class("CityBattleSelectArmy", require("game.uilayer.base.BaseLayer"))

function CityBattleSelectArmy:ctor(soldierData, armyUnitData, postArmy)
	CityBattleSelectArmy.super.ctor(self)
...
function CityBattleSelectArmy:setData()
	local curNum = 0
...
function CityBattleSelectArmy:addEvent()
	local cur = 0
    if self.armyUnitData ~= nil then
    ...
    local max = 0--cur + self.soldierData.num
    if max > self.maxArmy then
    	max = self.maxArmy
    end
